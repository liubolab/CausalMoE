...
import numpy as np
import sys
sys.path.append('/data/0shared/liubo/LLMTIME/llmcausal')
from layers.TS_Pos_Enc import *
    
# Cell
class CrossModal(nn.Module):
    def __init__(self, d_model, n_heads, d_k=None, d_v=None, d_ff=None, norm_feature=None,
        ...
        self.layers = nn.ModuleList([TSTEncoderLayer( d_model, n_heads=n_heads, d_k=d_k, d_v=d_v, d_ff=d_ff, norm=norm,
                                                      ...
                                                      pre_norm=pre_norm, store_attn=store_attn,index='cross_encoder',norm_feature=norm_feature) for i in range(n_layers)])
        self.res_attention = res_attention

    def forward(self, q:Tensor,k:Tensor,v:Tensor, key_padding_mask:Optional[Tensor]=None, attn_mask:Optional[Tensor]=None):
        '''
        q  [bs * nvars x (text_num) x d_model], time series encoding, (B,c,N)
        k  [bs * nvars x (text_num) x d_model], prompt encoding,(B,d,N)
        v  [bs * nvars x (text_num) x d_model],prompt encoding,(B,d,N)
        '''
        scores = None
        #print(q.shape,k.shape,v.shape)
        if self.res_attention:
            # output (B,c,N), scores (B,1,c,d)
...
class TSTEncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_k=None, d_v=None, d_ff=256, store_attn=False,
                 norm='LayerNorm', attn_dropout=0, dropout=0., bias=True, activation="gelu", res_attention=False, pre_norm=False,index='ts_encoder',norm_feature=None):
        super().__init__()
        ...
        if "batch" in norm.lower():
            ...
            if index=='ts_encoder':
                ...
                self.norm_attn = nn.LayerNorm(norm_feature)
