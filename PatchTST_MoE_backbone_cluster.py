...
# Cell
class PatchTST_MoE_cluster_patch(nn.Module):
    def __init__(self, c_in: int, c_out: int, target_window: int, patch_len: int, stride: int,
                 patch_num_in: int, patch_num_out: int,
                 ...
                 subtract_last=False, verbose: bool = False,
                 T_num_expert: int = 10, T_top_k: int = 2, F_num_expert: int = 10, F_top_k: int = 2, eta: int = 5, **kwargs):

        super().__init__()

        # Backbone
        self.project = nn.Linear(patch_num_in, patch_num_out)
        
        self.backbone = TSTiEncoder(c_in, patch_num=patch_num_out, patch_len=patch_len, max_seq_len=max_seq_len,
        ...
        # Cluster
        self.cluster = EDESC(d_model=d_model, n_clusters=T_num_expert, eta=eta, c_out=c_out,
                             patch_len=patch_len, stride=stride)

        # MoE
        # self.SaprseMoE = SparseMoE(n_embed=c_in * d_model, num_experts=T_num_expert, top_k=T_top_k)

        # Head
        self.head_nf = d_model * patch_num_out
        self.n_vars = c_in
        ...
        elif head_type == 'patch-wise':
            self.head = Patch_Head(self.individual, self.n_vars, d_model, patch_len, head_dropout=head_dropout)

    def forward(self, z):  # z: [bs x nvars x d_model x patch_num_in]

        # model
        time_z = self.backbone(z)  # z: [bs x nvars x d_model x patch_num_in]

        time_z = self.project(time_z)  # z: [bs x nvars x d_model x patch_num_out]
        ...
        s, h = self.cluster(time_cluster_input)

        bs, nvars, d_model, patch_num_out = time_z.shape
        time_z = time_z.permute(0, 3, 1, 2)
...
class Patch_Head(nn.Module):
    def __init__(self, individual, n_vars, d_model, patch_len, head_dropout=0):
        ...
        if self.individual:
            ...
            self.dropouts = nn.ModuleList()
            for i in range(self.n_vars):
                self.out_layer.append(nn.Linear(d_model, patch_len))
                self.dropouts.append(nn.Dropout(head_dropout))
        else:
            self.out_layer = nn.Linear(d_model, patch_len)
            self.dropout = nn.Dropout(head_dropout)

    def forward(self, x):  # x: [bs x nvars x d_model x patch_num_out]
        x = torch.reshape(x, (x.shape[0], x.shape[1], x.shape[3], x.shape[2]))  # z: [bs x nvars x patch_num_out x d_model]
        B = x.shape[0]
        m = x.shape[1]
        ...
        if self.individual:
            x_out = []
            for i in range(self.n_vars):
                z = self.out_layer[i](x[:, i, :, :])  # z: [bs x patch_num_out x d_model]
                z = self.dropouts[i](z)
                x_out.append(z)
            x = torch.stack(x_out, dim=1)  # x: [bs x nvars x patch_num_out x patch_len]
        else:
            # print('no individual')
            x = self.out_layer(x)  # x: [bs x nvars x patch_num_out x patch_len]
            x = self.dropout(x)

        # x = rearrange(x, 'b m n p -> b m (n p)', b=B, m=m)

        return x     # x: [bs x nvars x patch_num_out x patch_len]
